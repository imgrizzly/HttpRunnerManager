...
def case_info_logic(type=True, **kwargs):
    ...
    if 'request' not in test.keys():
        ...
        request_json = test.get('request').pop('request_json')
        request_params = test.get('request').pop('request_params')
        ...
        if request_json:
            test.get('request').setdefault('json', request_json)
        if request_params:
            data_dict = key_value_dict('data', **request_params)
            if not isinstance(data_dict, dict):
                return data_dict
        ...
        if request_data:
            ...
            if not isinstance(data_dict, dict):
                return data_dict
            test.get('request').setdefault('data', data_dict)
